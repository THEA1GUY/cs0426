...
### 🟡 Level 2: Retrieval Logic & Auth
1.  **Source Citations**: Modify the `Chat` API and UI to show *which* document chunks were used to generate the answer.
2.  **User Authentication**: Implement a secure login flow using Supabase Email Auth. (Currently being implemented).
3.  **Threshold Tuning**: Experiment with the `match_threshold` in the PostgreSQL function.
